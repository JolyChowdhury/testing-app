...
export default function RewardsContent() {
  ...
  const handleSignUpProgram = () => {
    navigate("/app/sign_up_program?tab=reward");
  };
  const handleWaysToPurchaseClick = () => {
    navigate("/app/ways_to_earn_purchased_reward?tab=reward");
  };
  const handleBirthdayPurchaseClick = () => {
    navigate("/app/birthday_reward?tab=reward");
  };
  const handleRewardFacebookClick = () => {
    navigate("/app/reward_facebook?tab=reward");
  };
  const handleRewardInstagramClick = () => {
    navigate("/app/reward_instagram");
  };
  const handleRewardTwitterClick = () => {
    navigate("/app/reward_twitter");
  };
    ...
    return (
      <InlineStack blockAlign="center" align="space-between">
        ...
        <InlineStack gap="400">
          ...
          <SwitchWithButton
            numberOfSwitches={numberOfSwitches}
            singleSwitchKey={singleSwitchKey}
          />
        </InlineStack>
      </InlineStack>
    );
...
//           <Button onClick={() => navigate("/app/ways_to_earn_purchased_reward?tab=reward")}>
//             Edit
//           </Button>
//         </div>
//         <AppProvider
...
//     >
//       <div style={{ marginRight: "20px" }}>
//         <Button onClick={() => navigate("/app/ways_to_earn_purchased_reward")}>
//           Edit
//         </Button>
//       </div>
//       <AppProvider
...
//   </div>
    <BlockStack gap="400">
        ...
        <TabsItem
          ...
          description="Offer a welcome reward upon a user registration"
          buttonLabel="Edit"
          handleButtonClick={handleSignUpProgram}
          numberofSwitches={1}
          switchKey="Sign up reward"
        />
        <Divider />
        <TabsItem
          heading="Rewards on Purchase"
          description="Offer to give reward points on purchase from the store"
          buttonLabel="Edit"
          handleButtonClick={handleWaysToPurchaseClick}
          numberofSwitches={2}
          switchKey="Purchase reward"
        />
        <Divider />
        <TabsItem
          heading="Birthday reward"
          description="Offer reward for the birthday of loyal customer"
          buttonLabel="Edit"
          handleButtonClick={handleBirthdayPurchaseClick}
          numberofSwitches={3}
          switchKey="Purchase reward"
        />
        <Divider />
            ...
            >
              <BlockStack gap="200">
                <SocialItem
                  ...
                  onClickEdit={handleRewardFacebookClick}
                  numberOfSwitches={5}
                  singleSwitchKey="facebook like switch"
                />
                <Divider />
                ...
                <SocialItem
                  ...
                  onClickEdit={handleRewardInstagramClick}
                  numberOfSwitches={6}
                  singleSwitchKey="instagram like switch"
                />
                <Divider />
              <SocialItem
                  icon={twitterIcon}
                  title="Like on Twitter"
                  onClickEdit={handleRewardTwitterClick}
                  numberOfSwitches={7}
                  singleSwitchKey="instagram like switch"
                />
                <Divider />
              <SocialItem
                  ...
                  onClickEdit={handleShareFacebookClick}
                  numberOfSwitches={7}
                  singleSwitchKey="Share on Facebook switch"
                />
                <Divider />
              <SocialItem
                  icon={ShareX}
                  title="Share on X"
                  onClickEdit={handleShareXClick}
                  numberOfSwitches={8}
                  singleSwitchKey="Share on X switch"
                />
              </BlockStack>
            </div>
          </Modal> 
